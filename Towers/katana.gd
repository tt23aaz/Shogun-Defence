...
# The katana starts as null because it does not know where to go yet
var target = null

# The speed of the katana
var Speed = 1000

# This is the name of the path that the katana shoots to
var path_name = ""

# This is the damage of the katana
var bullet_damage = 5


func _physics_process(_delta: float) -> void:
	# This gets the path node from the main scene and this is where the katana is shooting 
	...
	for i in range(path_spawner_node.get_child_count()):
		# Checks if the child node matches katana path name
	...
	# If there is no target found it will delete the katana
	if target == null:
		queue_free()
		return
	...
	# Katana then rotates to the target
	look_at(target)
	
	# Moves the katana using velocity values
	move_and_slide()

# Checks if the katana has touched the enemy
