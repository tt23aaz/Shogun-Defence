...
# Preloads the arrow scene
var Bullet = preload("res://Towers/arrow.tscn")

# The damage of each arrow hit will deal
...
# Removes all the Arrows in the bullet container
func clear_bullets():
...
func Shoot() -> void:
	...
	# Pass target path and damage data to the arrow 
	temp_bullet.path_name = path_name
	temp_bullet.bullet_damage = bullet_damage
	
	# Adds the arrow in the bullet container
	get_node("BulletContainer").add_child(temp_bullet)
	
	# The arrow is aimed and will shoot out of the left or right position 
	if current.global_position.x < global_position.x:
...
# When the timer finishes, fire at the target if it still exists then restarts timer for next shot
